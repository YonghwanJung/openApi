...
@Service
public class PopuEconActiveAdjustService extends YyyymmService{
	...
	public boolean doOneMonth(String yyyymm){
		...
		try {
			...
			int chkCnt = statPopulationRepository.selectExistChkStatPopulationEconActiveAdjust(yyyymm);

			if (chkCnt > 0) {
				statPopulationRepository.deleteStatPopulationEconActiveAdjust(yyyymm);
			}

			StringBuilder urlBuilder = new StringBuilder(
					"https://kosis.kr/openapi/statisticsData.do?method=getList&apiKey=ZGI2MGQ1NjYxZWI5MzE5ZDkzYjUwOWQ5YThjMjU1ODI=&format=json&jsonVD=Y&userStatsId=patron84/101/DT_1DA9001S/2/2/20221029094338&prdSe=M"); /*
																																																							 * URL
																																																							 */
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				if (StringUtils.isEmpty(orgC1)) {
					...
					dataMap.put("GENDER_CD", c1);
				}
				if (StringUtils.equals(orgC1, c1) == false) {
					System.out.println(dataMap.toString());
					statPopulationRepository.insertStatPopulationEconActiveAdjust(dataMap);
					orgC1 = c1;
					dataMap = new HashMap<>();
					dataMap.put("YYYYMM", prdDe);
					dataMap.put("GENDER_CD", c1);
