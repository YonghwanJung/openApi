...
@Service
@Slf4j
public class BadUnsoldHousingService extends YyyymmService{
	...
	public boolean doOneMonth(String yyyymm){
		...
		try {
			...
			int chkCnt = statRealEstateRepository.selectExistChkStatBadUnsoldHousing(yyyymm);

			if (chkCnt > 0) {
				statRealEstateRepository.deleteStatBadUnsoldHousing(yyyymm);
			}
			...
			urlBuilder.append("patron84/116/DT_MLTM_5328/2/2/20230126000349&prdSe=M"); /* URL*/
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				String regionNm = obj.getString("C1_NM");
				String dt = obj.getString("DT");
				String prdDe = obj.getString("PRD_DE");

				dataMap.put("YYYYMM", prdDe);
				dataMap.put("REGION_NM", regionNm);
				dataMap.put("CNT", dt);

				System.out.println(dataMap.toString());
				statRealEstateRepository.insertStatBadUnsoldHousing(dataMap);
