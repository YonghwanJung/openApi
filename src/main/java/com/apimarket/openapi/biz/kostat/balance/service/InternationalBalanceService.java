...
@Service
@Slf4j
public class InternationalBalanceService extends YyyymmService{
	...
	public boolean doOneMonth(String yyyymm){
		...
		try {
			...
			int chkCnt = statBalanceRepository.selectExistChkStatInternationalBalance(yyyymm);

			if (chkCnt > 0) {
				statBalanceRepository.deleteStatInternationalBalance(yyyymm);
			}
			...
			urlBuilder.append("patron84/301/DT_301Y013/2/2/20230118230414&prdSe=M"); /* URL*/
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				try {
					if ("13102134519ACC_CD.000000".equals(c1)) {
						dataMap.put("CURRENT_BALANCE", dt);
					} else if ("13102134519ACC_CD.100000".equals(c1)) {
						dataMap.put("GOODS_BALANCE", dt);
					} else if ("13102134519ACC_CD.200000".equals(c1)) {
						dataMap.put("SERVICE_BALANCE", dt);
					} else if ("13102134519ACC_CD.300000".equals(c1)) {
						dataMap.put("PRIMARY_INCOME_BALANCE", dt);
					} else if ("13102134519ACC_CD.400000".equals(c1)) {
						dataMap.put("TRANSFER_INCOME_BALANCE", dt);
					}
				} catch (Exception e) {
				...
				if (i == (jsonArray.length() - 1)) {
					...
					log.debug("2");
					statBalanceRepository.insertStatInternationalBalance(dataMap);
				}
